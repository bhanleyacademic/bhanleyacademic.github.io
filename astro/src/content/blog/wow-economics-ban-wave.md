...
At approximately 5:00 AM UTC on March 17, Blizzard executed a mass ban wave targeting accounts that had been exploiting an undisclosed in-game mechanic. These weren't just a handful of accounts — they were bot operations that had been continuously farming raw materials and listing them on the Auction House at rock-bottom prices.

I didn't know any of this when I looked at the data that morning. I just saw this:
...
Between the 04:15 snapshot and the 05:15 snapshot — a single hour — one item went from **164 active sellers to 39**. Its price jumped from **0.01 gold to 0.12 gold** — a 12× increase in 60 minutes.

That's not a gradual market shift. That's a supply collapse.

Across 76 different commodity items, the same pattern appeared simultaneously: listings fell sharply, prices spiked, and it all happened in the same one-hour window. That's the fingerprint of accounts being removed atomically, not of sellers deciding to raise prices.
...
Where $Q$ is quantity, $P$ is price, and the bars denote the average of before-and-after values. We use this "arc" formula rather than a simple before/after ratio so the result is symmetric — it doesn't matter which direction you measure the change.
...
**Inelastic.** Even after the price jumped 188%, buyers only reduced purchases by 120% — they still needed the item. Across all five items we tested, every single one came back inelastic. These are functional goods — crafting materials, consumables — not luxuries. Players buying them for raids and professions are going to buy them regardless of price fluctuations.

This mirrors real-world commodity markets. Gasoline demand is inelastic in the short run: people don't immediately stop driving when prices spike at the pump. They might adjust over months, but their immediate demand barely moves.
...
The index averaged around 100–125 in the week before the ban wave — some natural price drift from the base period, consistent with normal supply/demand fluctuations. Within 48 hours of the ban wave, the index **jumped 7.1%**, from a pre-event average of 122.3 to a post-event average of 131.0.

To put that in context: the US Federal Reserve considers a 2% *annual* inflation rate to be its target. The WoW economy experienced a 7% price level increase within two days, driven entirely by a supply shock.

---

## What This Tells Us About Real Economies

The WoW Auction House isn't a perfect replica of a real market — bots distort supply in ways that don't have direct analogues, and players' utility functions are different from consumers buying necessities. But the fundamental mechanics are surprisingly faithful:

**1. Supply shocks cause price spikes, and the size of the spike depends on elasticity.** This is textbook economics, but here we can watch it happen in real time at hourly resolution. In the real world, supply shocks like OPEC output cuts, semiconductor shortages, or port disruptions produce the same qualitative result — and the same inelastic demand response for essential goods.

**2. Enforcement actions are supply shocks.** When Blizzard bans bot accounts, it's economically equivalent to a government crackdown on illegal production removing supply from a market. The price effects are real and measurable, regardless of whether the removed suppliers were "legitimate."
...
One last note: the Corrupted Blood incident was studied after the fact, from player recollections and archived forum posts. We caught this ban wave in real time, in the data, while it was happening. That's what continuous data collection enables — and it's the same reason central banks, epidemiologists, and supply chain managers invest so heavily in real-time data infrastructure.
